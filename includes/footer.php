...
$footerTagline = getSiteSetting('footer_tagline', cms_default_setting('footer_tagline'));
$footerAddress = getSiteSetting('footer_address', cms_default_setting('footer_address'));
...
$navSuitesHref = getSiteSetting('nav_suites_href', cms_default_setting('nav_suites_href'));
$navDiningHref = getSiteSetting('nav_dining_href', cms_default_setting('nav_dining_href'));
$navExperienceHref = getSiteSetting('nav_experience_href', cms_default_setting('nav_experience_href'));
$navEventsHref = getSiteSetting('nav_events_href', cms_default_setting('nav_events_href'));
  ...
  <div class="max-w-[1280px] mx-auto px-6 lg:px-12">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-12 lg:gap-16 mb-16">
      ...
      <div class="min-w-0<?= empty($socialMediaList) ? ' h-[254px]' : '' ?>">
        <div class="site-brand-logo site-brand-logo--footer flex items-center justify-center md:justify-start mb-8 md:mb-10 min-h-[5rem] md:min-h-[7.5rem] py-2 md:py-3">
        ...
        <div class="flex gap-4">
          <?php foreach ($socialMediaList as $social): ?>
            <?php
              ...
              $svg = $platform !== '' ? social_platform_svg($platform) : '';
            ?>
            <?php if ($url !== '' && $svg !== ''): ?>
      ...
      <div class="min-w-0">
        ...
        <ul class="space-y-3 text-sm text-background-light/80">
          ...
          $footerExplore = [
              ['about', 'Our Story'],
              [$navSuitesHref, $navSuitesLabel],
              [$navDiningHref, $navDiningLabel],
              [$navExperienceHref, $navExperienceLabel],
              [$navEventsHref, $navEventsLabel],
              ['contact', 'Contact'],
          ];
          foreach ($footerExplore as $fe):
              ...
              ?>
          <li><a class="hover:text-champagne transition-colors" href="<?= e($resolvedHref) ?>"><?= e($lbl) ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
    ...
    <div class="border-t border-background-light/15 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-background-light/50">
      <div class="flex flex-wrap items-center justify-center md:justify-start gap-x-2 gap-y-2">
        ...
        <?php if (site_public_page_exists($hotelPolicySlug)): ?>
          <span class="mx-1 text-background-light/40">|</span>
          <a class="hover:text-champagne transition-colors whitespace-nowrap" href="<?= e(site_url($hotelPolicySlug)) ?>">Hotel Policy</a>
        <?php endif; ?>

        <?php if (site_is_valid_nav_href($footerPrivacyLink)): ?>
          <span class="mx-1 text-background-light/40">|</span>
          <a class="hover:text-champagne transition-colors whitespace-nowrap" href="<?= e($footerPrivacyLink) ?>">Privacy Policy</a>
        <?php endif; ?>

        <?php if (site_is_valid_nav_href($footerTermsLink)): ?>
          <span class="mx-1 text-background-light/40">|</span>
          <a class="hover:text-champagne transition-colors whitespace-nowrap" href="<?= e($footerTermsLink) ?>">Terms &amp; Conditions</a>
        <?php endif; ?>
      </div>
