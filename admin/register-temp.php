 ...
 * Optional hardening: set CMS_TEMP_REGISTER_SECRET to a random string, then add ?key=that-string to the URL.
 */
...
const CMS_TEMP_REGISTER_SECRET = '';
...
if (CMS_TEMP_REGISTER_SECRET !== '') {
    $provided = isset($_POST['reg_key']) ? (string)$_POST['reg_key'] : (string)($_GET['key'] ?? '');
    if ($provided === '' || !hash_equals(CMS_TEMP_REGISTER_SECRET, $provided)) {
        http_response_code(403);
        header('Content-Type: text/html; charset=UTF-8');
        echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Forbidden</title></head><body>';
        echo '<p>Missing or invalid key. Use <code>?key=…</code> matching <code>CMS_TEMP_REGISTER_SECRET</code> in <code>register-temp.php</code>.</p>';
        echo '</body></html>';
        exit;
    }
}
...
$registerAction = 'register-temp.php' . (CMS_TEMP_REGISTER_SECRET !== '' ? ('?key=' . rawurlencode(CMS_TEMP_REGISTER_SECRET)) : '');
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Temporary Admin Registration - <?= htmlspecialchars((string) getSetting('cms_product_name', cms_default_setting('cms_product_name')), ENT_QUOTES, 'UTF-8') ?></title>
  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <form method="POST" action="<?= htmlspecialchars($registerAction, ENT_QUOTES, 'UTF-8') ?>" class="login-form" autocomplete="off">
        ...
        <?php if (CMS_TEMP_REGISTER_SECRET !== ''): ?>
        <input type="hidden" name="reg_key" value="<?= htmlspecialchars(CMS_TEMP_REGISTER_SECRET, ENT_QUOTES, 'UTF-8') ?>">
        <?php endif; ?>
