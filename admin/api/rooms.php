 ...
 */
function resolveUniqueDisplayOrder(PDO $pdo, $requestedOrder, $excludeId = null) {
...
/**
 * Normalize free-form room metadata stored as a JSON object.
 */
function normalizeRoomMetaObject($input) {
    if (!is_array($input)) {
        return [];
    }
    ...
    foreach ($input as $key => $value) {
        $safeKey = preg_replace('/[^a-z0-9_-]/i', '', strtolower(trim((string)$key)));
        if ($safeKey === '' || strlen($safeKey) > 64) {
            continue;
        }
...
try {
    switch ($method) {
        ...
        case 'POST':
            ...
            $pdo->beginTransaction();
            $displayOrder = resolveUniqueDisplayOrder($pdo, $requestedDisplayOrder, null);
        ...
        case 'PUT':
            ...
            $pdo->beginTransaction();
            $displayOrder = resolveUniqueDisplayOrder($pdo, $requestedDisplayOrder, $id);
