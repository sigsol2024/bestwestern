...
if (defined('ROOM_FORM_ADD_MODE') && ROOM_FORM_ADD_MODE) {
  $id = 0;
} else {
    ...
    <div class="card-body card-body--stack">
        ...
        <div class="form-group">
          ...
          <div id="main_image_preview" class="image-preview" style="<?= !empty($room['main_image']) ? 'display:block;' : 'display:none;' ?>">
            <?php if (!empty($room['main_image'])): ?>
              <div class="site-media-thumb" style="position:relative; display:inline-block; margin-top:10px;">
        ...
        <div class="form-group">
          ...
          <div id="gallery_images_preview" class="image-preview" style="<?= !empty($room['gallery_images']) ? 'display:block;' : 'display:none;' ?>">
            <?php foreach (($room['gallery_images'] ?? []) as $i => $img): ?>
              <div class="site-media-thumb" data-i="<?= (int)$i ?>" style="position:relative; display:inline-block; margin:6px;">
        ...
        <div class="card-body card-body--stack">
          ...
          <div id="features-container">
            ...
            foreach ($featList as $f) {
                ...
                ?>
            <div class="form-row site-repeat-row" style="align-items: flex-end; gap: 8px;">
              <div class="form-group" style="flex: 1; margin-bottom: 0;">
                <input type="text" class="form-control js-feature-input" value="<?= sanitize($ft) ?>" placeholder="e.g. King bed, City view">
              </div>
              <button type="button" class="btn btn-outline btn-sm" onclick="removeRepeatRow(this)">Remove</button>
            </div>
            <?php } ?>
          </div>
      ...
      <div class="card card--nested">
        ...
        <div class="card-body card-body--stack">
          ...
          <button type="button" class="btn btn-outline btn-sm" onclick="addAmenityCard()">Add amenity</button>
        </div>
      </div>
      ...
      <div class="card card--nested">
        ...
        <div class="card-body card-body--stack">
          ...
          <button type="button" class="btn btn-outline btn-sm" onclick="addIncludedRow()">Add included item</button>
        </div>
      </div>
...
(function () {
  ...
  var pathTemplate = <?= json_encode(site_url('room-details', ['slug' => '__SLUG__'])) ?>;
...
function removeRepeatRow(btn) {
  var row = btn.closest('.site-repeat-row');
  if (row) row.remove();
}
...
function addFeatureRow() {
  ...
  wrap.className = 'form-row site-repeat-row';
    ...
    if (preview) {
      if (!clean.length) {
        ...
        preview.innerHTML = clean.map((p, i) =>
          '<div class="site-media-thumb" data-i="' + i + '" style="position:relative; display:inline-block; margin:6px;">' +
