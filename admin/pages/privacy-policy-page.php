...
$sections = [];
try {
    $stmt = $pdo->prepare("SELECT section_key, content FROM page_sections WHERE page = 'privacy-policy'");
    $stmt->execute();
...
$pageActiveSettingKey = 'page_active_privacy-policy';
$pageIsActive = ((string) getSetting($pageActiveSettingKey, cms_default_setting($pageActiveSettingKey, '1'))) === '1';
?>

<form id="privacyPolicyPageForm">
  <div class="card">
    <div class="card-header"><h2>Meta</h2></div>
    <div style="padding:20px;">
      <div class="form-group">
        <label for="page_title">Page title</label>
        <input type="text" id="page_title" name="page_title" value="<?= sanitize($sections['page_title'] ?? 'Privacy Policy') ?>">
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div style="padding:20px;">
      ...
      <div class="form-group">
        ...
        <input type="text" id="hero_title" name="hero_title" value="<?= sanitize($sections['hero_title'] ?? 'Privacy Policy') ?>">
      </div>
      <div class="form-group">
        <label for="hero_subtitle">Hero subtitle</label>
        <textarea id="hero_subtitle" name="hero_subtitle" rows="2"><?= sanitize($sections['hero_subtitle'] ?? 'How we collect, use, and protect your personal information.') ?></textarea>
      </div>
...
<script>
document.getElementById('privacyPolicyPageForm').addEventListener('submit', function (e) {
  e.preventDefault();
  savePageForm(this, 'privacy-policy', {}, { pageActiveSettingKey: 'page_active_privacy-policy' })
