  ...
  <?php else: ?>
    ...
    <div id="mediaViewGrid" class="media-grid">
      <?php foreach ($items as $row):
        $url = media_public_url($row['file_path'] ?? '');
        $id = (int)($row['id'] ?? 0);
        ?>
        <div class="media-card" data-id="<?= $id ?>">
          ...
          <div class="media-card__body">
            <p class="media-card__name" title="<?= sanitize($row['original_name'] ?? '') ?>"><?= sanitize(media_label_short($row['original_name'] ?? '', 10)) ?></p>
          ...
          <tbody>
            <?php foreach ($items as $row):
              ...
              <tr data-id="<?= $id ?>">
                ...
                <td title="<?= sanitize($row['original_name'] ?? '') ?>"><?= sanitize(media_label_short($row['original_name'] ?? '', 10)) ?></td>
...
(function () {
  ...
  function setView(mode) {
    ...
    try { localStorage.setItem('site_media_view', mode); } catch (e) {}
    updateBulkBar();
  }

  try {
    var saved = localStorage.getItem('site_media_view');
